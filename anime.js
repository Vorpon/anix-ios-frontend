...
function renderDetail(anime) {
    ...
    // НАСТРОЙКА АВТОНОМНОГО МУЛЬТИПЛЕЕРА (БЕЗ СБОЙНОГО БЭКЕНДА)
    const playerContainer = document.getElementById('kinobox-player');
    if (playerContainer) {
        playerContainer.innerHTML = `
            <div class="w-full h-full flex flex-col gap-3">
                <div class="flex gap-2 mb-1">
                    <button id="btn-collaps" class="bg-indigo-600 text-white text-xs font-bold px-3 py-1.5 rounded-lg border border-indigo-500 shadow-md transition-all">Плеер #1 (Collaps)</button>
                    <button id="btn-shiki" class="bg-zinc-900 text-zinc-400 text-xs font-medium px-3 py-1.5 rounded-lg border border-zinc-800 hover:border-zinc-700 transition-all">Плеер #2 (Vidsrc/Shiki)</button>
                </div>
        ...
        if (btnCollaps && btnShiki && playerFrame) {
            btnCollaps.onclick = () => {
                playerFrame.src = `https://api.collaps.org/embed/shikimori/${anime.id}`;
                btnCollaps.className = "bg-indigo-600 text-white text-xs font-bold px-3 py-1.5 rounded-lg border border-indigo-500 shadow-md";
                btnShiki.className = "bg-zinc-900 text-zinc-400 text-xs font-medium px-3 py-1.5 rounded-lg border border-zinc-800";
            };

            btnShiki.onclick = () => {
                // Если Касперский или провайдер блокирует Collaps, этот линк сработает без проблем
                playerFrame.src = `https://vidsrc.me/embed/anime?shikimori=${anime.id}`;
                btnShiki.className = "bg-indigo-600 text-white text-xs font-bold px-3 py-1.5 rounded-lg border border-indigo-500 shadow-md";
                btnCollaps.className = "bg-zinc-900 text-zinc-400 text-xs font-medium px-3 py-1.5 rounded-lg border border-zinc-800";
            };
        }
    }
}
